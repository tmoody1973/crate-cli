...
- **search_reviews** — Search 26 music publications (Pitchfork, The Quietus, Resident Advisor, Stereogum, BrooklynVegan, FACT, NME, NPR, The Guardian, Bandcamp Daily, AllMusic, The Wire, The FADER, Aquarium Drunkard, Boomkat, Paste Magazine, Exclaim!, PopMatters, and more) for album/artist reviews. Optionally extract full review text for deeper analysis. When Exa is available and initial results are sparse, automatically discovers additional related reviews via semantic similarity.
...
- **trace_influence_path** — Find a chain of influence connections between two artists (max depth 2-5). Uses Tavily (advanced depth) for direct connection search and Exa neural search for neighborhood expansion. Returns a formatted visualization.
- **find_bridge_artists** — Find artists that connect two genres or scenes. Uses Exa neural search for conceptual crossover discovery and Tavily for genre-specific searches. Scores candidates by how strongly they appear in both genre contexts.

**Influence methodology:** Use search_reviews to gather raw review text from 26 publications (auto-enriched with Exa semantic discovery when available), extract_influences to pull co-mentions from that text, trace_influence_path for direct "A influenced B" questions, and find_bridge_artists for genre-bridging discovery. Exa neural search powers conceptual queries (neighborhood expansion, crossover discovery) while Tavily handles keyword/domain-filtered searches. All Exa calls fall back to Tavily automatically if only TAVILY_API_KEY is set. Always cross-reference influence tool results with Last.fm similarity scores and MusicBrainz relationships for stronger evidence.
